...
if __name__ == "__main__":
    ...
    AR = 5
                # Wing aspect ratio
    aircraft.taper    = .8 # taper ratio
    aircraft.dihedral = 5  # Wing dihedral (degrees)
    aircraft.tau      = .1  # thickness-to-chord ratio

    
    # Tail parameters
    aircraft.Sh = .03 # Wing area of horizontal tail (m^2)
    aircraft.Sv = .04 # Wing area of vertical tail (m^2)

    # Fuselage parameters
    # aircraft.l_AR = 1 # Fuselage wingspan to length ratio (-)

    # Aerodynamic parameters
    aircraft.CLdes    = 1   # maximum CL wing will be designed to fly at (in cruise)
    aircraft.e0       = 1   # Span efficiency for straight level flight

    # Wing bending and material properties
    aircraft.dbmax    = .1 # tip displacement bending constraint
    aircraft.rhofoam  = 25.2     # kg/m^3. high load foam
    aircraft.Efoam    = 19.3E6  # Pa.     high load foam

    num_division = 20
    #enter tau values
    # tau_sweep = np.linspace(0.08, 0.12, 10)
    # lam_sweep = np.linspace(0.5, 1, 10)
    # z = []
    # for t in tau_sweep:
    #     for l in lam_sweep:
    #         aircraft.taper = l
    #         aircraft.tau = t
    #         z_v, _, _ = scan_ARS(aircraft=aircraft,
    #                 AR_start = 5.0,
    #                 AR_end = 15.0,
    #                 S_start = 0.1,
    #                 S_end = 0.7,
    ...
    # np.savez("data_DS4_test", z=z)
    scan_ARS(aircraft=aircraft,
                    AR_start = 1.0,
                    AR_end = 15.0,
                    S_start = 0.1,
                    S_end = 0.7,
